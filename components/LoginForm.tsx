...
export function LoginForm({ className, ...props }: React.ComponentProps<'div'>) {
    ...
    return (
        <>
            {/* Merge these hardcoded classes with the prop tailwind classes from args, and transfer
            over the parent's props */}
            ...
            {(state?.statusCode === 401 || state?.statusCode === 400) && (
                <div className="mt-10">
                    <AlertError errorStr={'Invalid email or password'} />
                </div>
            )}
        </>
    );
